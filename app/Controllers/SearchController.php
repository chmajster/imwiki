...
final class SearchController extends BaseController{
 ...
 public function index(Request $r):void{$uid=$this->requireAuth();$q=trim((string)$r->input('q',''));$results=$this->search->searchVisible($uid,$this->authz->isAdmin($uid),$q,75);echo$this->view->render('search.php',$this->common(['query'=>$q,'results'=>$results,'parsed'=>$this->search->parse($q),'savedSearches'=>$this->saved->all($uid)]));}
