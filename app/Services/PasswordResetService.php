...
final class PasswordResetService
{
    ...
    public function reset(string $token,string $password):int
    {
        ...
        $this->pdo->beginTransaction();try{$this->pdo->prepare("UPDATE `{$this->prefix}users` SET password_hash=?,force_password_change=0,updated_at=UTC_TIMESTAMP() WHERE id=?")->execute([password_hash($password,PASSWORD_DEFAULT),$uid]);$this->pdo->prepare("UPDATE `{$this->prefix}password_reset_tokens` SET used_at=UTC_TIMESTAMP() WHERE id=? AND used_at IS NULL")->execute([(int)$row['token_id']]);$this->sessions->revokeCredentialsForUser($uid,false);$this->pdo->commit();return $uid;}catch(\Throwable $e){if($this->pdo->inTransaction())$this->pdo->rollBack();throw $e;}
    }
}
