...
final class SchedulerService
{
    ...
    public function opportunistic(): array
    {
        ...
        try{
            rewind($fh);$last=(int)trim((string)stream_get_contents($fh));if($last>time()-3600)return ['ran'=>false];
            $queued=$this->digest->opportunistic(25);$clean=$this->retention->cleanup();
            ftruncate($fh,0);rewind($fh);fwrite($fh,(string)time());fflush($fh);
            return ['ran'=>true,'digests'=>$queued,'cleanup'=>$clean];
