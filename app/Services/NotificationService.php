...
final class NotificationService
{
    ...
    {
        if ($actorId !== null && $actorId === $userId && !str_starts_with($type, 'security.')) return 0;
    ...
    public function notifyWatchers(string $event, int $actorId, int $pageId, int $spaceId, string $url, array $payload = []): void
    {
        ...
        if ($event === 'page.updated') $resources[] = ['space',$spaceId];
        $seen = [];
    ...
    {
        $prefs=$this->preferences($userId);if(!$prefs['in_app'])return 0;
        $count=0;$offset=0;$batch=250;
        do{
            $stmt=$this->pdo->prepare("SELECT id,type,target_type,target_id FROM `{$this->prefix}notifications` WHERE user_id=:uid AND read_at IS NULL ORDER BY id DESC LIMIT :limit OFFSET :offset");
            $stmt->bindValue(':uid',$userId,PDO::PARAM_INT);$stmt->bindValue(':limit',$batch,PDO::PARAM_INT);$stmt->bindValue(':offset',$offset,PDO::PARAM_INT);$stmt->execute();$rows=$stmt->fetchAll();
            foreach($rows as $row)if($this->categoryAllowed($prefs,(string)($row['type']??''))&&$this->visible($userId,$row['target_type']?:null,$row['target_id']!==null?(int)$row['target_id']:null))$count++;
            $offset+=$batch;
    ...
    {
        $page = max(1, $page);$perPage = max(1, min(50, $perPage));$wantedStart = ($page - 1) * $perPage;$visible = [];$offset = 0;$batch = 100;$prefs=$this->preferences($userId);
        while (count($visible) < $wantedStart + $perPage + 1) {
            ...
            $stmt->bindValue(':uid',$userId,PDO::PARAM_INT);$stmt->bindValue(':limit',$batch,PDO::PARAM_INT);$stmt->bindValue(':offset',$offset,PDO::PARAM_INT);$stmt->execute();$rows = $stmt->fetchAll();if (!$rows) break;
            foreach ($rows as $row) {
                if (!$prefs['in_app']||!$this->categoryAllowed($prefs,(string)$row['type']) || !$this->visible($userId, $row['target_type'] ?: null, $row['target_id'] !== null ? (int)$row['target_id'] : null)) continue;
                $row['payload'] = $row['payload_json'] ? (json_decode((string)$row['payload_json'], true) ?: []) : [];$visible[] = $row;
            }
            if (count($rows) < $batch) break;$offset += $batch;
        }
        $slice = array_slice($visible, $wantedStart, $perPage);
        return ['items'=>$slice,'page'=>$page,'per_page'=>$perPage,'has_previous'=>$page>1,'has_next'=>count($visible)>$wantedStart+$perPage];
    }

    public function markRead(int $userId, int $notificationId): void {$stmt=$this->pdo->prepare("UPDATE `{$this->prefix}notifications` SET read_at=COALESCE(read_at,UTC_TIMESTAMP()) WHERE id=? AND user_id=?");$stmt->execute([$notificationId,$userId]);}
    public function markAllRead(int $userId): void {$stmt=$this->pdo->prepare("UPDATE `{$this->prefix}notifications` SET read_at=UTC_TIMESTAMP() WHERE user_id=? AND read_at IS NULL");$stmt->execute([$userId]);}

    public function preferences(int $userId): array
    {
        if(isset($this->preferencesCache[$userId]))return $this->preferencesCache[$userId];
        ...
        return $this->preferencesCache[$userId]=['in_app'=>(bool)($data['in_app']??true),'email_mode'=>in_array((string)($data['email_mode']??'none'),['none','immediate','daily','weekly'],true)?(string)($data['email_mode']??'none'):'none','categories'=>array_replace($defaultCats,is_array($data['categories']??null)?$data['categories']:[])];
    }

    public function savePreferences(int $userId,bool $inApp,string $emailMode,array $categories): void
    {
        if(!in_array($emailMode,['none','immediate','daily','weekly'],true))$emailMode='none';$allowed=['mentions','comments','replies','watched_pages','watched_spaces','tasks','security'];$selected=array_fill_keys(array_intersect($allowed,$categories),true);$cat=[];foreach($allowed as $key)$cat[$key]=isset($selected[$key]);$payload=['in_app'=>$inApp,'email_mode'=>$emailMode,'categories'=>$cat];$json=json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
        $stmt=$this->pdo->prepare("INSERT INTO `{$this->prefix}user_preferences` (user_id,notification_json,updated_at) VALUES (?,?,UTC_TIMESTAMP()) ON DUPLICATE KEY UPDATE notification_json=VALUES(notification_json),updated_at=UTC_TIMESTAMP()");$stmt->execute([$userId,$json]);$this->preferencesCache[$userId]=$payload;
    }
    ...
    private function visible(int $userId, ?string $targetType, ?int $targetId): bool
    {
        ...
        if ($targetType === 'page') {$page=$this->pageById($targetId);return $page!==null&&$this->authz->canViewPage($userId,$page);}
        if ($targetType === 'space') return $this->authz->canViewSpace($userId,$targetId);
        if ($targetType === 'task') {$stmt=$this->pdo->prepare("SELECT page_id FROM `{$this->prefix}tasks` WHERE id=?");$stmt->execute([$targetId]);$pageId=(int)($stmt->fetchColumn()?:0);if($pageId<=0)return false;$page=$this->pageById($pageId);return $page!==null&&$this->authz->canViewPage($userId,$page);}
        return true;
    }
}
