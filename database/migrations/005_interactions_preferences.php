...
return static function(PDO $pdo,string $prefix):void{
    ...
    if(!$hasColumn($prefix.'templates','space_id')){
        $pdo->exec("ALTER TABLE `{$prefix}templates` ADD COLUMN space_id BIGINT UNSIGNED NULL AFTER id, ADD COLUMN labels_json JSON NULL AFTER content, ADD COLUMN properties_json JSON NULL AFTER labels_json, ADD COLUMN archived_at DATETIME NULL AFTER is_system");
        $fkTemplatesSpace='fk_imwiki_'.substr(hash('sha256',$prefix.'templates_space'),0,24);
        $pdo->exec("ALTER TABLE `{$prefix}templates` ADD CONSTRAINT `{$fkTemplatesSpace}` FOREIGN KEY(space_id) REFERENCES `{$prefix}spaces`(id) ON DELETE CASCADE");
    }
