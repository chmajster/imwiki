...
$assert($r['status']===200,'Login POST');
$r=$http($base.'/spaces');$assert($r['status']===200&&str_contains($r['body'],'Przestrzenie'),'Authenticated spaces page');
$r=$http($base.'/spaces','POST',['_csrf'=>$csrf($r['body']),'name'=>'Release Smoke Space','key'=>'SMOKE','description'=>'Release ZIP test']);
$spaceCount=(int)$pdo->query("SELECT COUNT(*) FROM spaces WHERE space_key='SMOKE' AND deleted_at IS NULL")->fetchColumn();
$assert($spaceCount===1,'Space create DB state, HTTP '.$r['status']);
$r=$http($base.'/spaces/SMOKE');$assert($r['status']===200&&str_contains($r['body'],'Release Smoke Space'),'Space create browser state');
$r=$http($base.'/spaces/SMOKE/pages/create');$assert($r['status']===200&&str_contains($r['body'],'Nowa strona'),'Page create form');
...
$pageCount=(int)$pdo->query("SELECT COUNT(*) FROM pages WHERE title='Release Artifact Page' AND deleted_at IS NULL")->fetchColumn();
$assert($pageCount===1,'Page create DB state, HTTP '.$r['status']);
$r=$http($base.'/spaces/SMOKE');$assert(str_contains($r['body'],'Release Artifact Page'),'Page create browser state');
